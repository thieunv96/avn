...
#
# Usage: install.sh [--dry-run] [--force] [TARGET_DIR]
...
# Sanity: required source files present (so we fail clearly if run from the wrong dir).
for f in "CLAUDE.md" ".claude/settings.json" ".claude/hooks/bash-guard.sh"; do
  if [ ! -f "$SRC/$f" ]; then
    echo "Error: baseline source is missing $f — run install.sh from the baseline repo." >&2
    exit 1
  fi
done
